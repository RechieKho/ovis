...
namespace ovis::ast
{
    ...
    namespace implementation
    {
        ...
        class uint64_builder_node final : public xint_builder_node<t_generator_type>
        {
        public:
        ...
        public:
            explicit uint64_builder_node(uint32_type p_value, optional_token_type p_token = optional_token_type())
                : base_type(false, std::move(p_token)), m_value(p_value) {}

            explicit uint64_builder_node(const uint32_builder_node_type &p_node)
                : base_type(false, std::move(p_node.get_token())), m_value(p_node.get()) {}

            explicit uint64_builder_node(const uint16_builder_node_type &p_node)
                : base_type(false, std::move(p_node.get_token())), m_value(p_node.get()) {}

            explicit uint64_builder_node(const uint8_builder_node_type &p_node)
                : base_type(false, std::move(p_node.get_token())), m_value(p_node.get()) {}
            ...
            {
                ATTEMPT_RETURN_UNSIGNED_UNIFORM_BINARY_OPERATION(
